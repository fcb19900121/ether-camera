...
#endif
#define delay_ms(ms)  vTaskDelay((ms > portTICK_PERIOD_MS ? ms/ portTICK_RATE_MS : 1))
#define OV2710_SUPPORT_NUM CONFIG_CAMERA_OV2710_MAX_SUPPORT
#define OV2710_AEC_TARGET_DEFAULT (0x45)  // AEC target luminance: 0x45=69 (darker), 0x50=80, 0x60=96 (default), 0x70=112 (brighter)
...
static const esp_cam_sensor_isp_info_t ov2710_isp_info[] = {
    ...
    {   // [3] 1280x720 50fps
        .isp_v1_info = {
            .version = SENSOR_ISP_INFO_VERSION_DEFAULT,
            .pclk = 80000000,
            .vts = 893,   // VTS for 50fps: 80000000/(1792*50)=893
            .hts = 1792,
            .bayer_type = ESP_CAM_SENSOR_BAYER_BGGR,
        },
    },
...
static const esp_cam_sensor_format_t ov2710_format_info[] = {
    /* For MIPI - 1 lane configurations (2-lane has issues) */
    {   // [0]
    ...
    {   // [3]
        .name = "MIPI_1lane_24Minput_RAW10_1280x720_50fps",
        .format = ESP_CAM_SENSOR_PIXFORMAT_RAW10,
        .port = ESP_CAM_SENSOR_MIPI_CSI,
        .xclk = OV2710_MCLK,
        .width = 1280,
        .height = 720,
        .regs = init_reglist_MIPI_1lane_1280_720_50fps,
        .regs_size = ARRAY_SIZE(init_reglist_MIPI_1lane_1280_720_50fps),
        .fps = 50,
        .isp_info = &ov2710_isp_info[3],
        .mipi_info = {
            .mipi_clk = 800000000,
            .lane_num = 1,
            .line_sync_en = false,
...
static esp_err_t ov2710_query_para_desc(esp_cam_sensor_device_t *dev, esp_cam_sensor_param_desc_t *qdesc)
{
    ...
    case ESP_CAM_SENSOR_GAIN:
        ...
        qdesc->number.minimum = 16;   // 1x gain (register uses fixed-point, 16=1x)
        qdesc->number.maximum = 64;   // 4x gain max (limits noise/JPEG size)
        qdesc->number.step = 1;
        qdesc->default_value = 16;
        break;
    case ESP_CAM_SENSOR_EXPOSURE_VAL:
        // Exposure in lines; 720p@25fps VTS=1786, max=VTS-4=1782
        qdesc->type = ESP_CAM_SENSOR_PARAM_TYPE_NUMBER;
        qdesc->number.minimum = 1;
        qdesc->number.maximum = 1782;
        qdesc->number.step = 1;
        qdesc->default_value = 500;
        break;
...
static esp_err_t ov2710_get_para_value(esp_cam_sensor_device_t *dev, uint32_t id, void *arg, size_t size)
{
    ...
    switch (id) {
    case ESP_CAM_SENSOR_GAIN: {
        uint8_t gain_h = 0, gain_l = 0;
        ESP_RETURN_ON_FALSE(size == 4, ESP_ERR_INVALID_ARG, TAG, "Para size err");
        ov2710_read(dev->sccb_handle, OV2710_REG_AEC_AGC_ADJ_MSB, &gain_h);
        ov2710_read(dev->sccb_handle, OV2710_REG_AEC_AGC_ADJ_LSB, &gain_l);
        *(uint32_t *)arg = ((uint32_t)(gain_h & 0x03) << 8) | gain_l;
        break;
    }
    case ESP_CAM_SENSOR_EXPOSURE_VAL: {
        uint8_t expo_h = 0, expo_m = 0, expo_l = 0;
        ESP_RETURN_ON_FALSE(size == 4, ESP_ERR_INVALID_ARG, TAG, "Para size err");
        ov2710_read(dev->sccb_handle, 0x3500, &expo_h);
        ov2710_read(dev->sccb_handle, 0x3501, &expo_m);
        ov2710_read(dev->sccb_handle, 0x3502, &expo_l);
        uint32_t expo_16ths = ((uint32_t)(expo_h & 0x0f) << 16) | ((uint32_t)expo_m << 8) | expo_l;
        *(uint32_t *)arg = expo_16ths >> 4;
        break;
    }
    case ESP_CAM_SENSOR_AE_LEVEL: {
        ESP_RETURN_ON_FALSE(size == 4, ESP_ERR_INVALID_ARG, TAG, "Para size err");
...
static esp_err_t ov2710_set_para_value(esp_cam_sensor_device_t *dev, uint32_t id, const void *arg, size_t size)
{
    ...
    switch (id) {
    case ESP_CAM_SENSOR_GAIN: {
        // Write AGC gain registers directly; 0x3a00 AEC/AGC bits are managed by init regs
        ESP_RETURN_ON_FALSE(size == 4, ESP_ERR_INVALID_ARG, TAG, "Para size err");
        uint32_t gain_val = *(const uint32_t *)arg;
        gain_val = gain_val < 16 ? 16 : (gain_val > 64 ? 64 : gain_val);  // 4x gain max
        ov2710_write(dev->sccb_handle, OV2710_REG_AEC_AGC_ADJ_MSB, (gain_val >> 8) & 0x03);
        ov2710_write(dev->sccb_handle, OV2710_REG_AEC_AGC_ADJ_LSB, gain_val & 0xFF);
        break;
    }
    case ESP_CAM_SENSOR_EXPOSURE_VAL: {
        // Write exposure registers; 0x3a00 AEC/AGC bits are managed by init regs
        ESP_RETURN_ON_FALSE(size == 4, ESP_ERR_INVALID_ARG, TAG, "Para size err");
        uint32_t lines = *(const uint32_t *)arg;
        lines = lines < 1 ? 1 : (lines > 1782 ? 1782 : lines);
        uint32_t expo_16ths = lines << 4;
        ov2710_write(dev->sccb_handle, 0x3500, (expo_16ths >> 16) & 0x0f);
        ov2710_write(dev->sccb_handle, 0x3501, (expo_16ths >> 8) & 0xff);
        ov2710_write(dev->sccb_handle, 0x3502, expo_16ths & 0xff);
        break;
    }
    case ESP_CAM_SENSOR_VFLIP: {
        int *value = (int *)arg;
